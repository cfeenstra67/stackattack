...
import { CapacityProviderInput, ClusterInput, ClusterOutput, ClusterWithCapacityProvider, getCapacityProviderId, getClusterAttributes } from "./cluster.js";
import { Network, VpcInput } from "./vpc.js";
...
export function taskDefinition(ctx: Context, args: TaskDefinitionArgs) {
  ...
  const appContainer: awsNative.types.input.ecs.TaskDefinitionContainerDefinitionArgs = {
    ...
    healthCheck: args.healthcheck ? {
      command: [
        'CMD-SHELL',
        args.healthcheck.command
      ],
      interval: args.healthcheck.interval ?? 10,
      startPeriod: args.healthcheck.startPeriod ?? 30,
      retries: args.healthcheck.retries ?? 3
    } : undefined,
    environment: pulumi.output(args.env ?? {}).apply((env) => Object.entries(env).map(([name, value]) => ({ name, value }))),
  };

  const containers: awsNative.types.input.ecs.TaskDefinitionContainerDefinitionArgs[] = [appContainer];
  if (args.init) {
    const initEnv = pulumi.all([args.env, args.init.env]).apply(([env1, env2]) => ({
      ...env1,
      ...env2
    }));
    ...
    const initContainer: awsNative.types.input.ecs.TaskDefinitionContainerDefinitionArgs = {
      ...
      environment: initEnv.apply((env) => Object.entries(env).map(([name, value]) => ({ name, value }))),
    };
...
export function service(ctx: Context, args: ServiceArgs) {
  ...
  const service = new aws.ecs.Service(ctx.id(), {
