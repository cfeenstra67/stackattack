...
import { getVpcId, Network, VpcInput } from "./vpc.js";
...
export type ClusterInput = pulumi.Input<string> | pulumi.Input<aws.ecs.Cluster> | pulumi.Input<aws.ecs.GetClusterResult> | ClusterOutput;
...
export function getClusterAttributes(input: ClusterInput): pulumi.Output<aws.ecs.Cluster | aws.ecs.GetClusterResult> {
  return pulumi.output(input).apply((value) => {
    if (typeof value === 'string') {
...
export type CapacityProviderInput = pulumi.Input<string> | pulumi.Input<aws.ecs.CapacityProvider>;

export function getCapacityProviderId(input: CapacityProviderInput): pulumi.Output<string> {
  return pulumi.output(input).apply((value) => {
    if (typeof value === 'string') {
      return pulumi.output(value);
    }
...
export function clusterInstanceRole(ctx: Context, args?: ClusterInstanceRoleArgs) {
  if (!args?.noPrefix) {
  ...
  new aws.iam.RolePolicyAttachment(
    ctx.id('cloudwatch-agent-policy-attach'),
    {
      role: ecsInstanceRole.name,
...
export function getInstanceTypeArchitecture(instanceType: pulumi.Input<string>): pulumi.Output<string> {
...
export function clusterSecurityGroup(ctx: Context, args: ClusterSecurityGroupArgs) {
  if (!args.noPrefix) {
  ...
  new aws.ec2.SecurityGroupRule(ctx.id('ingress-ssh'), {
    type: 'ingress',
...
export function clusterInstanceInitScript({ cluster, paramName }: ClusterInstanceInitScriptArgs): pulumi.Output<string> {
...
export function clusterCapacity(ctx: Context, args: ClusterCapacityArgs) {
  ...
  const launchConfig = new aws.ec2.LaunchConfiguration(ctx.id('launch-config'), {
    imageId: ami.id,
    instanceType,
    iamInstanceProfile: profile,
    userData: clusterInstanceInitScript({ cluster: cluster.id, paramName: cloudWatchConfigParam.name }),
  ...
  const autoScalingGroup = new aws.autoscaling.Group(ctx.id('auto-scaling'), {
    ...
    tags: Object.entries(ctx.tags()).map(([key, value]) => ({ key, value, propagateAtLaunch: true }))
  }, {
...
export function cluster(ctx: Context, args: ClusterArgs): ClusterOutput {
  ...
  return { cluster, capacityProvider: capacity.capacityProvider, autoScalingGroup: capacity.autoScalingGroup };
}
